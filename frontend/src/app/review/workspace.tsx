      ...
      <div className="review-workspace">
        <section className="viewer">
          <div className="video-stage">
            ...
            <svg className="rv-annotation-canvas" viewBox="0 0 100 100" preserveAspectRatio="none"><defs><marker id="rv-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#ffcf5a" /></marker></defs>{view.annotations.flatMap((annotation) => annotation.elements.map((element, index) => <AnnotationShape key={`${annotation.id}-${index}`} annotationId={annotation.id} element={element} canDelete={Boolean(view.workspaceId && view.projectId && view.version)} canMove={!annotationTool && annotation.author_user_id === currentUserId} onMove={(dx, dy) => startAnnotation(async () => { const result = await updateAnnotationElementsAction(view.workspaceId!, view.projectId!, view.version!.id, annotation.id, annotation.elements.map((item) => ({ ...item, geometry: moveGeometry(item, dx, dy) }))); if (result.error) setAnnotationError(result.error); })} onDelete={() => startAnnotation(async () => { const result = await deleteAnnotationAction(view.workspaceId!, view.projectId!, view.version!.id, annotation.id); if (result.error) setAnnotationError(result.error); })} />))}</svg>
...
function AnnotationShape({ element, onDelete, onMove, canDelete, canMove }: { annotationId: string; element: AnnotationElement; onDelete: () => void; onMove: (dx: number, dy: number) => void; canDelete: boolean; canMove: boolean }) {
  ...
  return <g className="rv-shape" onPointerDown={(event) => { if (!canMove) return; const bounds = event.currentTarget.ownerSVGElement!.getBoundingClientRect(); drag.current = { x: event.clientX, y: event.clientY, width: bounds.width, height: bounds.height }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerUp={(event) => { if (!drag.current) return; const { x, y, width, height } = drag.current; drag.current = null; onMove((event.clientX - x) / width, (event.clientY - y) / height); }} onDoubleClick={canDelete ? onDelete : undefined}>{shape}<title>{canMove ? "Drag to move · double-click to delete" : element.element_type}</title></g>;
}

function moveGeometry(element: AnnotationElement, dx: number, dy: number): Record<string, unknown> {
  const clamp = (value: number) => Math.max(0, Math.min(1, value)); const g = element.geometry;
  if (element.element_type === "POINT" || element.element_type === "TEXT") return { ...g, x: clamp(Number(g.x) + dx), y: clamp(Number(g.y) + dy) };
  if (element.element_type === "RECTANGLE" || element.element_type === "ELLIPSE") return { ...g, x: Math.max(0, Math.min(1 - Number(g.width), Number(g.x) + dx)), y: Math.max(0, Math.min(1 - Number(g.height), Number(g.y) + dy)) };
  if (element.element_type === "ARROW") { const start = g.start as Record<string, number>, end = g.end as Record<string, number>; const safeDx = Math.max(-Math.min(start.x, end.x), Math.min(1 - Math.max(start.x, end.x), dx)); const safeDy = Math.max(-Math.min(start.y, end.y), Math.min(1 - Math.max(start.y, end.y), dy)); return { start: { x: start.x + safeDx, y: start.y + safeDy }, end: { x: end.x + safeDx, y: end.y + safeDy } }; }
  const points = g.points as { x: number; y: number }[]; const safeDx = Math.max(-Math.min(...points.map((p) => p.x)), Math.min(1 - Math.max(...points.map((p) => p.x)), dx)); const safeDy = Math.max(-Math.min(...points.map((p) => p.y)), Math.min(1 - Math.max(...points.map((p) => p.y)), dy)); return { points: points.map((point) => ({ x: point.x + safeDx, y: point.y + safeDy })) };
}
